...
ScavTrap::~ScavTrap(){
	std::cout << "Derived--> ScavTrap destrucot was called" << std::endl;
}
...
ScavTrap::ScavTrap(std::string str): ClapTrap(str) {  // the name can be initialized by the based constructor
	...
	_energyPoints = 50;
	_attackDamage = 20;
//	std::cout << "derived parameter constructor called" << std::endl;
	std::cout << "Derived--> ScavTrap parameter constructor  Name set : " << _name << std::endl;
...
ScavTrap &ScavTrap::operator=(const ScavTrap &obj) {
	...
	std::cout << "Derived--> ScavTrap assigment opeator created" << std::endl;
	return *this;
}
...
void ScavTrap::attack(const std::string &target) {
	...
	{
		std::cout<<" Derived--> ScavTrap Not enough hit point or energi for an attack" << std::endl;
		return ;
	}
